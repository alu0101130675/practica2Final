    ...
    for (int l = j - 1; l <= j + 1; l++)
    {
      ...
      if (k != i || l != j)
        ...
        if (board.get_Malla()[k * board.get_Columnas() + l]->getState() == 'a')
          contadorAdulta++;
        else if (board.get_Malla()[k * board.get_Columnas() + l]->getState() == 'e')
          contadorhuevo++;
        else if (board.get_Malla()[k * board.get_Columnas() + l]->getState() == 'l')
          contadorLarva++;
        else if (board.get_Malla()[k * board.get_Columnas() + l]->getState() == 'p')
          contadorPupas++;
